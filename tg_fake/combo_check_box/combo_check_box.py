...
from PyQt5.QtWidgets import QWidget, QComboBox, QLineEdit, QListView, QApplication, QScrollBar, QScrollArea
    ...
    def wrapped(self, *args, **kwargs):
        if self.vars["showTextLock"]:
            ...
            self.vars["listViewModel"].item(0).setCheckState(
                2 if l == l_ else 0 if l == 0 else 1)
...
class QComboCheckBox(QComboBox):
    ...
    def select_reverse(self):
        # 反转选择
        if self.vars["listViewModel"].item(0).checkState() == 0:
            print(123)
            self.select_all()
        elif self.vars["listViewModel"].item(0).checkState() == 2:
            self.select_clear()
        else:
    ...
    def select_all(self):
        ...
        for row in range(0, self.vars["listViewModel"].rowCount()):
            self.vars["listViewModel"].item(row).setCheckState(2)

    @show_text
    def select_clear(self):
        # 全不选
        for row in range(0, self.vars["listViewModel"].rowCount()):
            self.vars["listViewModel"].item(row).setCheckState(0)
    ...
    def is_all(self):
        # 判断是否是全选
        return True if self.vars["listViewModel"].item(0).checkState() == 2 else False
    ...
    def __show_selected(self, index):
        if not self.vars["lock"]:
            if index == 0:
                if self.vars["listViewModel"].item(0).checkState() == 2:
                    self.select_clear()
                else:
                    self.select_all()
            else:
...
if __name__ == '__main__':
    ...
    app.setStyle("Fusion")
    main_window = QComboCheckBox()
    main_window.add_items(("a", "4d", "d", "5d", "d", "4d", "d", "d", "d65", "65d", "d98", "d", "65d"))
